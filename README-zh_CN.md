...
## 自定义URL路径
当上传到额外图床时，您可以指定完整的URL路径（域名部分除外）：

1. 发送图片时添加说明文字(caption)来指定路径（例如：`path/to/image.jpg`）
2. 如果指定的路径已存在文件，机器人会询问是否要覆盖现有图片
3. 您可以确认或取消上传
...
- `CLOUDFLARE_KV_NAMESPACE_ID`: 你的 Cloudflare KV 存储的命名空间 ID。
- `CLOUDFLARE_BUCKET_NAME`: 你的 Cloudflare 存储的桶名称。
- `R2_CUSTOM_DOMAIN`: 你的 Cloudflare R2 存储的自定义域名。
- `TG_BOT_TOKEN`: 你的 Telegram 机器人令牌。
- `TG_WEBHOOK_SECRET_TOKEN`: Telegram webhook 的密钥令牌。
- `TG_BOT_OWNER_USERNAME`: Telegram 机器人所有者的用户名。
- `TG_BOT_ALLOW_ANYONE`: 允许任何人使用 Telegram 机器人的配置。
- `B2_KEY_ID`: 你的 Backblaze B2 密钥 ID。
- `B2_SECRET_KEY`: 你的 Backblaze B2 密钥。
- `B2_ENDPOINT`: 你的 Backblaze B2 存储的端点。
- `B2_BUCKET`: 你的 Backblaze B2 存储的桶名称。
- `B2_CUSTOM_DOMAIN`: 你的 Backblaze B2 存储的自定义域名。
